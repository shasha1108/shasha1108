...
15 年里反复验证了一件事：任何重复的工作都可以被拆解、被规则化、被自动化。

从测试自动化脚本，到数据平台让几百人免于重复取数，再到 AI 内容创作全流程工作流——每一次转身，都是在消灭上一份工作里的重复部分。

现在做的是同一件事，只是工具不同：把业务拆成规则，把规则写成系统，让 AI 去执行。交付的不是一段 prompt，而是一套能跑的判断逻辑加兜底方案。
